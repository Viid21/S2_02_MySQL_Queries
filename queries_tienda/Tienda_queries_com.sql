...
SELECT p.nombre AS nombre_producto, p.precio FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE lower(f.nombre) LIKE '%w%';
...
SELECT nombre AS nombre_producto, (SELECT nombre FROM fabricante WHERE codigo = producto.codigo_fabricante) AS nombre_fabricante FROM producto WHERE codigo_fabricante = (SELECT codigo FROM fabricante WHERE nombre = 'Lenovo');
